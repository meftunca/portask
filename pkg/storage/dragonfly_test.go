...
func TestDragonflyStorageIntegration(t *testing.T) {
	...
	storage, err := NewDragonflyStorage(config)
	...
	t.Run("Test message storage and retrieval", func(t *testing.T) {
		...
		messages, err := storage.Fetch(ctx, message.Topic, 0, 0, 10)
		require.NoError(t, err)
	...
	t.Run("Test message cleanup and deletion", func(t *testing.T) {
		...
		messages, err := storage.Fetch(ctx, testTopic, 0, 0, 10)
		require.NoError(t, err)
		...
		err = storage.Cleanup(ctx, retentionPolicy)
		require.NoError(t, err)
		...
		messagesAfterCleanup, err := storage.Fetch(ctx, testTopic, 0, 0, 10)
		require.NoError(t, err)
		...
		if len(messageIDs) > 0 {
			err := storage.Delete(ctx, messageIDs[0])
			require.NoError(t, err)
			t.Log("✅ Individual message deletion successful")

			// Verify deletion
			deletedMsg, err := storage.FetchByID(ctx, messageIDs[0])
		...
		if len(messageIDs) > 2 {
			...
			err := storage.DeleteBatch(ctx, batchIDs)
			require.NoError(t, err)
			t.Logf("✅ Batch deletion successful for %d messages", len(batchIDs))
		}

		// Final message count
		finalMessages, err := storage.Fetch(ctx, testTopic, 0, 0, 10)
		require.NoError(t, err)
	...
	t.Run("Test TTL and automatic expiration", func(t *testing.T) {
		...
		// Store both messages
		err := storage.Store(ctx, oldMessage)
		require.NoError(t, err)
		err = storage.Store(ctx, recentMessage)
		require.NoError(t, err)
	...
	t.Run("Test high-volume cleanup performance", func(t *testing.T) {
		...
		// Verify all stored
		messages, err := storage.Fetch(ctx, testTopic, 0, 0, messageCount+10)
		require.NoError(t, err)
		...
		// Check final count
		time.Sleep(100 * time.Millisecond)
		finalMessages, err := storage.Fetch(ctx, testTopic, 0, 0, messageCount+10)
		require.NoError(t, err)
...
func TestDragonflyStorageReliability(t *testing.T) {
	...
	t.Run("Test basic storage operations", func(t *testing.T) {
		...
		testTopic := types.TopicName("basic_test")
	...
	t.Run("Test topic management", func(t *testing.T) {
		testTopic := types.TopicName("mgmt_test")
